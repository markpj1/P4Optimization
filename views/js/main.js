...
var latestKnownScrollY = 0, //place checker
    ticking = true;

function onScroll() {
  latestKnownScrollY = window.scrollY; //creates a scroll #
  requestTick(); //invoke function
}

function requestTick() { //invokes RAF.
  if (!ticking) {
...
function updatePositions() {
    ...
    var currentScrollY = latestKnownScrollY / 1250; //removed from loop 
    var phase;
    for (var i = 0; i < items.length; i++) {
      phase = Math.sin(currentScrollY + (i % 5));
      items[i].style.right = items[i].basicLeft + 100 * phase + 'px';
    }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 34; i++) { //reduced number of elements to 4 * 8 = 34
  ...
  //updatePositions();
  window.requestAnimationFrame(updatePositions); //added requestAnimationFrame() not sure if this does anything.
});